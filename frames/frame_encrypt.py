...
class FrameEncrypt(tk.Toplevel):
    def __init__(self, root=None, helper=None):
        ...
        self.methods_list.insert(0, 'AES')
        self.methods_list.insert(1, 'Triple DES')

        self.generate_key_label = tk.Label(self, text=f'Key: {State.generated_key_filename or ""}')
        self.generate_key_label.grid(row=2, column=0, columnspan=4, padx=10, pady=(0, 0), sticky='w')

        self.generate_key_btn = tk.Button(self, text='New key', command=self.helper.generate_random_key)
        self.generate_key_btn.grid(row=3, column=0, columnspan=2, padx=(10, 5), pady=(0, 10), sticky='we')

        self.generate_key_btn = tk.Button(self, text='Choose key', command=self.helper.choose_key)
        self.generate_key_btn.grid(row=3, column=2, columnspan=2, padx=(0, 10), pady=(0, 10), sticky='we')
